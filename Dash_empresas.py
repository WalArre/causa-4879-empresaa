...
def login():
    ...
    if not st.session_state["autenticado"]:
        st.markdown("<h2 style='text-align: center; color: white;'>🚔 Sala de Situación PFA</h2>", unsafe_allow_html=True)
...
if login():
    ...
    # 2. ESTÉTICA DE TARJETAS EMPRESARIALES
    st.markdown("""
        <style>
        .stApp { background-color: #0b131e; color: #e0e0e0; }
        .company-card {
            background-color: #162636; border-radius: 12px; padding: 20px;
        ...
        .subject-report { 
            font-size: 0.75rem; font-style: italic; color: #3498db; 
        ...
        with tab_g:
            ...
            for i, emp in enumerate(empresas):
                with cols[i % 3]:
                    ...
                    # CONSTRUCCIÓN DEL BLOQUE HTML COMPLETO POR EMPRESA
                    html_content = f"""
                    <div class="company-card">
                        <div class="header-box">
                            <div class="company-title">{emp}</div>
                            <div style="color:#7f8c8d; font-size:0.85rem;">CUIT: {cuit_label}</div>
                        </div>
                    """
                    ...
                    for _, row in data_emp.iterrows():
                        ...
                        html_content += f"""
                        <div class="subject-block {border}">
