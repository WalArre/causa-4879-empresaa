...
# 1. SEGURIDAD DE ACCESO (Nueva Clave)
def login():
...
if login():
    ...
    st.markdown("""
        ...
        .company-card {
            ...
            box-shadow: 0 8px 16px rgba(0,0,0,0.6); min-height: 480px;
        }
    ...
    try:
        ...
        with col_s:
            query = st.text_input("🔍 Buscador Global (Empresa, Sujeto, DNI o CUIT):", "").lower()
        with col_f:
            status_filtro = st.multiselect("Filtrar estados:", ["Verde", "Amarillo", "Rojo"], default=["Verde", "Amarillo", "Rojo"])

        # Filtrado lógico por empresa (si un integrante coincide, se muestra toda la empresa)
        ...
        with tab2:
            ...
            cols = st.columns(3) # 3 Columnas para mejor legibilidad en TV
            ...
            for i, emp in enumerate(empresas):
                with cols[i % 3]:
                    ...
                    # Generación manual del HTML para evitar errores de renderizado
                    st.markdown(f'<div class="company-card"><div class="company-header-box"><div class="company-title">{emp}</div><div style="color:#7f8c8d; font-size:0.8rem;">CUIT: {cuit_label}</div></div>', unsafe_allow_html=True)
        ...
        st.error(f"Falla crítica de datos: {e}")
